...
\item
  ...
  \item
    Covariate distributions match between treated and control
    conditional on the propensity score.
  \end{itemize}
\item
  ...
  \item
    ...
    all of \(X\) does. Ten covariates collapse to one scalar: the curse
    of dimensionality defeated, for \emph{conditioning}.
  \end{itemize}
\end{itemize}
\end{frame}
...
\item
  ...
  \texttt{ROA} near \(0.49\). After weighting all are under \(0.02\).
  This is the plot referees want, and where most papers stop.
\item
...
\item
  ...
  nobody is left to carry it: \textbf{this is where the \(ATT\) fails},
  and why it needs only \(e(X) < 1\) while the \(ATE\) needs
  \(0 < e(X) < 1\).
\end{itemize}
\end{frame}
...
\item
  ...
  subclassification, matching, the propensity score. Each refuses to
  compare a treated unit with no counterpart; common support is the
  binding constraint.
\item
...
\item
  ...
  silent the model speaks anyway: OLS imputes the counterfactual by
  \textbf{extrapolating} into regions with no comparable units.
\item
...
\item
  ...
  \]
\item
  Multivariate coefficients are \textbf{not} simple scaled covariances.
  But Frisch--Waugh--Lovell (Frisch and Waugh 1933; Lovell 1963) says
  they are scaled covariances of something else.
\end{itemize}
\end{frame}
...
\item
  ...
  coefficient becomes a picture whose slope is
...
\item
  ...
  unnecessary.''
\item
  Three assumptions, not one. You have met unconfoundedness; the other
  two arrived silently, in the act of writing the equation.
\end{itemize}
\end{frame}
...
\item
  \(\gamma_{11}\) is the age--earnings profile for \textbf{college
  graduates}; \(\gamma_{01}\) is that profile for the same people
  \textbf{had they not gone}.
\item
  \(\gamma_{11} \neq \gamma_{01}\) \textbf{is} heterogeneous treatment
  effects --- the return to treatment varying with \(X\).
\item
  In finance this is unremarkable: the effect of index inclusion, a
  rating change or a governance shock varies with firm size. If it does,
  \(\gamma_{11} \neq \gamma_{01}\).
\end{itemize}
\end{frame}
...
\item
  A2 is strong. Neither matching nor the propensity score required it:
  they average heterogeneous effects rather than assuming them away.
\end{itemize}
\end{frame}
...
\tightlist
\item
  The problem was never OLS. It was \textbf{one specification}: additive
  and separable.
\item
  The \textbf{Kitagawa--Oaxaca--Blinder decomposition} splits a mean gap
  into differences in \emph{endowments} (education, experience) and
  differences in the \emph{returns} to them.
\item
  Słoczyński (2015, 2020) showed the decomposition is also a
  \textbf{treatment effects estimator}, identifying both the \(ATE\) and
  the \(ATT\). That is what ``regression adjustment'' means.
\end{itemize}
\end{frame}
...
\item
  ...
  appears in both potential outcomes. What remains is the interaction
  --- exactly the term the canonical model omitted.
\end{itemize}
\end{frame}
...
\item
  ...
  differs in age and GPA composition, and the effect varies with both.
\item
...
\item
  At exactly \(50/50\) the canonical model lands on the \(ATE\) ---
  \textbf{by coincidence of the design, not by validity}.
\end{itemize}
\end{frame}
...
\item
  Inside the model is a model of the missing potential outcome. The true
  \(Y^0\) contained age, age\(^2\), GPA, GPA\(^2\) and
  age\(\,\cdot\,\)GPA --- so the treatment had to be interacted with
  \textbf{all five}.
\item
  Drop one interaction and it breaks. With the effect also varying in
  age\(\,\cdot\,\)GPA, the fully interacted model returns \(2576\)
  against a true \(ATE\) of \(2575\); omitting the single
  \(D \cdot (\text{age}\cdot\text{gpa})\) term returns \(2368\).
\item
...
\item
  ...
  the propensity score asked for this. Regression adjustment removes it,
  at the cost of interacting treatment with everything.
\item
...
\item
  \textbf{It delivers only one number.} One treatment indicator, one
  coefficient. Regression adjustment delivers the \(ATE\) and the
  \(ATT\) from the same fit.
\end{enumerate}
\end{frame}
...
\item
  ...
  unchanged, and the collider \(F\) of Lecture 3a §2 is the warning.
\end{itemize}
...
\item
  This is why the Lecture 3a §3 distinction matters: two assumptions
  were failing, and only one is checkable.
\end{itemize}
\end{frame}
...
\item
  ...
  The stage is blind to \(Y\), which is what makes the specification
  search legitimate.
\item
  ...
  \(2.27\).
\item
  \textbf{IPW} reweights into a pseudo-experiment: \(1/\hat e\) and
  \(1/(1-\hat e)\) build the population; \(\hat e/(1-\hat e)\) on
  controls alone rebuilds the \emph{treated} distribution. Watch the
  tails, and normalize.
\item
  ...
  randomization when blocking was available, so pruning can
...
\item
  ...
  \(D\)}, and Słoczyński's weights show the resulting coefficient is
  generally neither the \(ATE\) nor the \(ATT\).
\item
