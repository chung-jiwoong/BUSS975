...
\section{1. Why Graphs?}\label{why-graphs}
...
\section{2. DAG Notation}\label{dag-notation}
...
\begin{frame}{Conditioning is not intervening}
\protect\phantomsection\label{conditioning-is-not-intervening}
\begin{itemize}
\tightlist
\item
  \textbf{Conditioning on \(X\)} filters: ``among firms that
  \emph{happen} to have \(X = x\).'' We keep a subpopulation, which may
  be special in ways we cannot see.
\item
  \textbf{Intervening on \(X\)} sets: ``if we \emph{assigned} \(X = x\)
  to every firm.'' The population stays fixed; only \(X\) and its
  consequences move.
\item
  Prediction needs the first. Causal inference needs the second.
\item
  In graph terms, intervening \textbf{deletes every arrow into \(X\)}:
  \(X\) is no longer caused by its parents; it is caused by us.

  \begin{itemize}
  \tightlist
  \item
    That is what randomization does physically. No arrows in, nothing to
    block --- no controls needed for identification.
  \end{itemize}
\item
  The rest of the machinery answers one question: \textbf{when does
  conditioning on the right \(Z\) reproduce intervening?}
\end{itemize}
\end{frame}
...
\item
  ...
  its credit rating.
\item
  \textbf{Conditioning}: among AAA issuers, default is rare. Ratings are
  superb \emph{predictors} --- selling this number is a profitable
  business.
\item
  \textbf{Intervening}: suppose a regulator relabelled every junk issuer
  AAA. Default rates would barely move. The label is not the cash flows.
\item
  Conditioning \textbf{selects} firms that were already creditworthy;
  Intervening \textbf{changes the label} and leaves creditworthiness
  untouched.\footnote{The causal effect is not exactly zero. Kisgen and Strahan 2010: when the SEC certified DBRS in 2003, yields moved 39bp per notch --- the label alone, with fundamentals unchanged.}
\end{itemize}
...
\section{3. Three Elemental
Structures}\label{three-elemental-structures}
...
\item
  ...
  picture.
\item
  Example: firm quality drives both whether a firm has a lending
...
\item
  Example: skill and luck are independent, but both determine whether a
  fund survives. Study only survivors and skill and luck become
  correlated.
\item
...
\section{4. Backdoor Paths and the Backdoor
...
\section{5. Collider Bias}\label{collider-bias}
...
\item
  ...
  \end{itemize}
\item
  The correlation is manufactured entirely by \textbf{who is in the
  sample}.
\end{itemize}
\end{frame}
...
\tightlist
\item
  You do not need to type a variable into a regression to condition on
  it.
\item
...
\begin{frame}{Discrimination and collider bias}
\protect\phantomsection\label{discrimination-and-collider-bias}
\begin{itemize}
\tightlist
\item
  Gender discrimination is often denied like this: \emph{condition on
  occupation and the wage gap shrinks or disappears} --- within the same
  job, women are paid the same. \textbf{But what if occupation is itself
  a consequence of discrimination?}
\item
  Suppose every woman is ``treated'' with discrimination and no man is.
  Firms respond in \textbf{two ways}: they pay her less, \emph{and} they
  do not promote her --- so she is paid less again.
\end{itemize}
...
\tightlist
\item
  Men and women are \textbf{equally productive}, so there is no arrow
  \(F \to Y\). We are not after the effect of \emph{female} on earnings;
  we are after the effect of \textbf{discrimination}.
\end{itemize}
\end{frame}

\begin{frame}{What controlling for occupation actually does}
\protect\phantomsection\label{what-controlling-for-occupation-actually-does}
Three paths run from \(D\) to \(Y\):

\begin{enumerate}
\tightlist
\item
  \(D \to Y\) --- the \textbf{direct} effect. The causal path of
  interest.
\item
  \(D \to O \to Y\) --- \textbf{mediation}, not a backdoor path:
  discrimination worsens the job, and the worse job pays less.
\item
  \(D \to O \leftarrow A \to Y\) --- contains the \textbf{collider}
  \(O\), so it is \emph{closed} to begin with.
\end{enumerate}
...
\tightlist
\item
  Regress \(Y\) on \(D\) alone and you get the \textbf{total effect}:
  the direct effect plus the part mediated by occupational sorting.
\item
  Now add occupation, to ``compare men and women in similar jobs.'' Two
  things happen at once:

  \begin{itemize}
  \tightlist
  \item
    it \textbf{closes} the mediation channel \(D \to O \to Y\) ---
    reasonable, if the direct effect is what you want;
  \item
    it \textbf{opens} path 3, because conditioning on a collider opens
    it. The control ironically introduces a new pattern of bias.
  \end{itemize}
\item
  To shut path 3 again you would have to control for occupation
  \textbf{and ability} --- and ability is unobserved. So this design
  \textbf{does not satisfy the backdoor criterion}.
\end{itemize}
\end{frame}

\begin{frame}[fragile]{The simulation}
\protect\phantomsection\label{the-simulation}
\begin{Shaded}
\begin{Highlighting}[]
\FunctionTok{set.seed}\NormalTok{(}\DecValTok{975}\NormalTok{)}
\NormalTok{n              }\OtherTok{\textless{}{-}} \DecValTok{10000}
\NormalTok{female         }\OtherTok{\textless{}{-}} \FunctionTok{as.integer}\NormalTok{(}\FunctionTok{runif}\NormalTok{(n) }\SpecialCharTok{\textgreater{}=} \FloatTok{0.5}\NormalTok{)   }\CommentTok{\# half the population is female}
\NormalTok{ability        }\OtherTok{\textless{}{-}} \FunctionTok{rnorm}\NormalTok{(n)                      }\CommentTok{\# independent of gender; UNOBSERVED}
\NormalTok{discrimination }\OtherTok{\textless{}{-}}\NormalTok{ female                        }\CommentTok{\# all women experience discrimination}
\NormalTok{occupation     }\OtherTok{\textless{}{-}} \DecValTok{1} \SpecialCharTok{+} \DecValTok{2}\SpecialCharTok{*}\NormalTok{ability }\SpecialCharTok{{-}} \DecValTok{2}\SpecialCharTok{*}\NormalTok{discrimination }\SpecialCharTok{+} \FunctionTok{rnorm}\NormalTok{(n)}
\NormalTok{wage           }\OtherTok{\textless{}{-}} \DecValTok{1} \SpecialCharTok{{-}} \DecValTok{1}\SpecialCharTok{*}\NormalTok{discrimination }\SpecialCharTok{+} \DecValTok{1}\SpecialCharTok{*}\NormalTok{occupation }\SpecialCharTok{+} \DecValTok{2}\SpecialCharTok{*}\NormalTok{ability }\SpecialCharTok{+} \FunctionTok{rnorm}\NormalTok{(n)}

\NormalTok{b }\OtherTok{\textless{}{-}} \ControlFlowTok{function}\NormalTok{(f) }\FunctionTok{unname}\NormalTok{(}\FunctionTok{round}\NormalTok{(}\FunctionTok{coef}\NormalTok{(}\FunctionTok{lm}\NormalTok{(f))[}\StringTok{"discrimination"}\NormalTok{], }\DecValTok{3}\NormalTok{))}
\FunctionTok{c}\NormalTok{(}\AttributeTok{unconditional =} \FunctionTok{b}\NormalTok{(wage }\SpecialCharTok{\textasciitilde{}}\NormalTok{ discrimination),}
  \AttributeTok{add\_occupation =} \FunctionTok{b}\NormalTok{(wage }\SpecialCharTok{\textasciitilde{}}\NormalTok{ discrimination }\SpecialCharTok{+}\NormalTok{ occupation),}
  \AttributeTok{add\_occ\_ability =} \FunctionTok{b}\NormalTok{(wage }\SpecialCharTok{\textasciitilde{}}\NormalTok{ discrimination }\SpecialCharTok{+}\NormalTok{ occupation }\SpecialCharTok{+}\NormalTok{ ability))}
\end{Highlighting}
\end{Shaded}

\begin{verbatim}
  unconditional  add_occupation add_occ_ability 
         -2.983           0.591          -0.996 
\end{verbatim}
...
\tightlist
\item
  Discrimination is \textbf{hard-coded}: \(-1\) directly on wages, and
  \(-2\) on occupation. True direct effect \(=-1\); true total effect
  \(= -1 + 1\times(-2) = -3\).
\end{itemize}
\end{frame}

\begin{frame}{Reading the three regressions}
\protect\phantomsection\label{reading-the-three-regressions}
\begin{itemize}
\tightlist
\item
  \textbf{Unconditional \(\approx -3\).} A large negative effect --- the
  combination of the direct effect and the indirect effect through
  occupation. This is the \emph{total} effect, and it is correct if that
  is what you want.
\item
  \textbf{Add occupation \(\approx +0.6\).} \textbf{The sign flips: it
  literally becomes positive.} We know this is wrong, because the direct
  effect was hard-coded at \(-1\). Conditioning on the collider \(O\)
  opened \(D \to O \leftarrow A \to Y\), and the spurious path is strong
  enough to pervert the entire relationship.
\item
  \textbf{Add occupation and ability \(\approx -1\).} Now we recover the
  direct effect, because conditioning on \(A\) closes the path that
  conditioning on \(O\) opened.
\item
  The rescue only worked because ability was \textbf{observable in the
  simulation}. In real data it is not --- which is the whole point.
  ``Control for occupation'' is not the fix; it is the bug.
\end{itemize}
\end{frame}

\section{6. Bad Controls}\label{bad-controls}
...
\tightlist
\item
  The operational rule: \textbf{controls must be determined before
  treatment}.
\item
...
\section{7. Neutral Controls and
...
\item
  ...
  \(0.130\). It absorbs the variation in \(D\) you were relying on.
\item
...
\section{8. What DAGs Cannot Do}\label{what-dags-cannot-do}
