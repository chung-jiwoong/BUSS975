...
Lecture 3b takes the same assumption further: collapse \(X\) to one
number, weight instead of match, then compare with regression.
\end{frame}
...
\tightlist
\item
  Lecture 2 gave a rule for \textbf{which} variables to condition on:
...
\item
  ...
  \tightlist
  \item
    What if the variables that close the backdoors are
...
\item
  Subclassification, matching, propensity scores and regression are all
  answers to ``how.'' They differ less than their names suggest.
\end{itemize}
\end{frame}
...
\tightlist
\item
  Nothing here is an assumption; it holds whatever you do.
\item
  \textbf{A regression cannot delete the last two terms.} No functional
  form, no fixed effects, no clustering choice touches them.
\item
...
\item
  These came from the \textbf{assignment mechanism}, not the estimator.
  We then \emph{inserted} them into a statistical model.
\end{itemize}
...
\item
  ...
  mechanism.
\item
  Confusing them is the most common error in empirical finance: treating
  a long control list as an identification strategy.
\item
  Unconfoundedness is this lecture's mechanism: treatment assigned
  \textbf{as if at random within strata of \(X\)}.
\item
  Everything that follows is a statistical model, credible \emph{only}
  if you can defend that mechanism.
\item
...
\item
  ...
  of surviving (\(Y\))?
\item
  Raw comparison: \(62.5\%\) of first-class passengers survived versus
  \(27.1\%\) of everyone else --- a gap of \(35.4\) points.
\item
  But the \emph{``women and children first''} rule was actually
  enforced, and first-class cabins were closer to the boat deck.
\item
  \textbf{Sex} and \textbf{age} drive both who bought first class and
  who reached a lifeboat. Textbook confounding.
\item
  The data are real, complete, small enough to compute by hand, and the
  confounders are undisputed.
\end{itemize}
\end{frame}
...
\item
  ...
  is \emph{part of} the effect, not a confounder.
\end{itemize}
\end{frame}
...
\item
  \(S\) predicts the outcome but not treatment: free precision. The
  honest case for non-confounder controls.
\item
  \(M\) is subtle. Conditioning on it asks: \emph{did first class help,
  holding fixed the main way it helped?} Near zero --- and the wrong
  question.
\item
...
\tightlist
\item
  Analyst coverage is the finance collider: it responds to \emph{both}
  the financing event and firm performance.
\item
  The drawn amount is the mediator a referee will \emph{ask} for ---
  ``surely control for how much they borrowed.'' That removes the
  channel you are measuring.
\item
  The last row has no Titanic counterpart, and that is the whole
  problem. On the Titanic we know the confounders; in finance we rarely
  do.
\end{itemize}
\end{frame}
...
\item
  ...
  outcome.} These are confounders or precision-improvers like \(S\), and
  \emph{probably not colliders} --- a variable measured before treatment
...
\tightlist
\item
  Rule 2 does most of the work; rule 3 is most often violated in
  finance.
\item
  ``Pre-treatment'' does the heavy lifting in rule 2: a timing argument,
  and timing is checkable.
\end{itemize}
\end{frame}
...
\tightlist
\item
  A polio vaccine. The three parameters answer three different policy
  questions.
  ...
  \end{itemize}
\item
  They coincide only under constant effects. With heterogeneity they
...
\tightlist
\item
  The third row is the \emph{expansion} question --- almost never the
  one your data answers well.
\item
  Papers routinely estimate the \(ATT\) then draw conclusions requiring
  the \(ATE\) or \(ATU\). Watch for it in referee reports --- including
  your own.
\end{itemize}
\end{frame}
...
\item
  Among units with the \emph{same} \(X\), treatment is as good as
  randomly assigned: \textbf{conditional randomization}.
\item
...
\item
  ...
  substitution.
\item
  Switching estimators cannot rescue a design: all lean on the first
  equality, none can test it. \textbf{Identification} credibility is
  common to all.
\item
  They differ downstream --- functional form, extrapolation, which
  strata get weight. Those are \textbf{estimation} differences; Lecture
  3b §3 returns to them.
\end{itemize}
\end{frame}
...
\tightlist
\item
  Read literally: firms with identical observables \textbf{flip a coin}
  over a major financial decision.
\item
...
\item
  ...
  statistics.
\item
  Not a reason never to use these methods --- a reason to state why
  \emph{your} setting escapes it.
\end{itemize}
\end{frame}
...
\item
  ...
  knowledge the relevant \(X\) may be measurable. Whether it is depends
  on the setting, not econometrics.
\item
  \textbf{Objectives differ.} The decision-maker optimizes something
  \emph{other than your outcome}: a firm choosing a credit line
  optimizes liquidity and covenant slack, not the investment measure you
  study.
\end{enumerate}
...
\item
  It also says when the design \emph{fails}: when the agent optimizes
  precisely your outcome.
\end{itemize}
\end{frame}
...
\tightlist
\item
  \(ATE(x)\) is the effect \textbf{within stratum \(x\)} --- the CATE.
  The last line averages those with \textbf{population} weights.
\item
  \textbf{Unconfoundedness is used twice}, for \(Y^1\) and for \(Y^0\).
  That is why the \(ATE\) needs the \emph{strong} version.
\end{itemize}
\end{frame}
...
\item
  ...
  treatment.
\item
  Firms may \emph{anticipate their own gains} \(Y^1\) and select on
  them, but not on how they would have done anyway.

  \begin{itemize}
  \tightlist
  \item
    Often defensible: a firm knows its expected benefit from a credit
    line better than its counterfactual path without one.
  \end{itemize}
\item
...
\item
  ...
  treated} --- makes it the \(ATT\): strata with no treated units get
  zero weight.
\item
...
\item
  ...
  survival odds.
\item
  But first class held a very different mix of people. Sex and age are
  confounders, and observable.
\item
...
\item
  Composition: \(76\%\) of passengers were adult males --- the
  \emph{smallest} effect --- but only \(54\%\) of first class.
\item
  The last stratum holds \textbf{one} first-class female child. Remember
  her.
\end{itemize}
\end{frame}
...
\item
  The stratum still exists --- 44 girls travelled in other classes ---
  but has \textbf{no treated unit}. The difference is undefined.
\end{itemize}
...
\tightlist
\item
  One passenger of 2,201 destroys two of the three parameters. The
...
\item
  Non-parametric methods \textbf{break down honestly}: they simply
  cannot compute the estimate.
\item
  Regression \textbf{does not break down}: it extrapolates across empty
  cells using functional form, and reports a number with a standard
  error.
\end{itemize}
\end{frame}
...
\item
  ...
  does it \textbf{explicitly}, unit by unit --- its main virtue.
\item
  Start from the \(ATT\). Exactly \textbf{one} term is unavailable:
\end{itemize}
...
\tightlist
\item
  The first term is the treated sample mean. Matching builds a stand-in
  for the second.
\end{itemize}
\end{frame}
...
\tightlist
\item
  The missing quantity is now written entirely in terms of control
  units.
\item
...
\item
  ...
  breaking ties arbitrarily injects the analyst's coin flip.
  \textless!-- - \(M_i\) also varies when a caliper binds: a treated
  unit with no control inside the caliper contributes nothing, and
  \(N_T\) must then count only the units you actually matched.
\item
...
\item
  ...
  subclassification give the \textbf{same answer} --- the same
...
\item
  Same fatal problem: with a continuous covariate, \textbf{exact matches
...
\item
  The gap you accept is the \textbf{matching discrepancy}; §8 is about
  what it costs.
\item
  ...
  \item
    ...
    to drop a treated unit than match it to something unlike it.
  \end{itemize}
\end{itemize}
\end{frame}
...
\tightlist
\item
  If size and leverage are highly correlated, Euclidean distance
  double-counts what is essentially one dimension.
\end{itemize}
\end{frame}
...
\item
  ...
  control may serve several treated units, and matches were chosen
  \emph{using the data}.
\item
  Conventional standard errors treat the matched sample as randomly
  drawn. It was not, and they understate uncertainty.
\item
  Abadie and Imbens (2008) show conventional standard errors are invalid
...
\item
  Check what your software reports \emph{by default}: several packages
  give the naive standard error unless told otherwise.
\end{itemize}
\end{frame}
...
\tightlist
\item
  Term (a) is the \textbf{sample variance of the \(\hat\tau_i\)}. Alone
  it is the without-replacement formula: with \(K_j \in \{0,1\}\), term
  (b) vanishes.
\item
...
\item
  ...
  \textbf{Drop it and you get Abadie--Imbens}, Stata's default --- so
  report the second command of each pair.
\item
...
\tightlist
\item
  \texttt{age} has standard deviation \(9.1\), \texttt{gpa} only
  \(0.57\), so raw Euclidean distance \textbf{matches on age and ignores
  gpa} --- the scale problem from §6. Every trainee gets a
  \emph{different} control: \(K_j \leq 1\), term (b) is zero, and both
  standard errors coincide.
\item
  Mahalanobis makes \texttt{gpa} count --- and \texttt{gpa} is noise.
  \textbf{Three controls serve all ten trainees}; one is used five
  times.
\item
...
\tightlist
\item
  The second equality on each line \textbf{is} unconfoundedness: it lets
  a regression fitted on controls speak about treated units.
\item
...
\item
  ...
  normals: noise, and they average away.
\item
  Row (3) does not. Its summands are \textbf{signed}: if the nearest
  control is systematically older or smaller, every term carries the
  same sign.
\end{itemize}
\end{frame}
...
\item
  For the \(ATT\) it is \(Y^0\) that is missing and imputed, so only
  \(\mu^0\) has to be modelled.
\end{itemize}
\end{frame}
...
\item
  ...
  \(Y^0\) is \emph{observed}, then transplanted. Unconfoundedness
  permits the transplant.
\end{itemize}
\end{frame}
...
\item
  So \(\hat\mu^0\) must be accurate \emph{over the gap}, not globally.
  Get the level wrong everywhere and the correction is unaffected:
  \(\hat\alpha\) cancels.
\item
  \textbf{This is why bias correction is more credible than regression
  alone.} Regression extrapolates \(\hat\mu^0\) across the whole
  covariate space; bias correction interpolates across a gap matching
  already made small.
\item
  Division of labour: \textbf{matching does the heavy lifting on
  comparability; regression cleans up the remainder.}
\item
  The worse your matches, the more work the regression does --- and the
  more the method degenerates into regression.
\end{itemize}
\end{frame}
...
\tightlist
\item
  Exact matches get \textbf{zero} correction. Only the two bad matches
  move.
\item
...
\item
  Four points and one outlier. The ``small'' regression is doing
  enormous work --- the method's exposure.
\end{itemize}
\end{frame}
...
\item
  It is honest about its assumption: you must name a functional form for
  \(\hat\mu^0\), and the ten-worker example shows how much that moves
  the answer.
\item
  It does \textbf{not} repair the matches.
\item
  Report the raw and bias-corrected estimates side by side. A large gap
  is a \textbf{diagnostic}: your matches were poor and the outcome model
  is load-bearing.
\end{itemize}
\end{frame}
...
\item
  ...
  randomization \emph{within} strata of \(X\). Untestable; defended
  institutionally, and Imbens's third defence is strongest in finance.
\item
  \textbf{Common support} is a \emph{separate} assumption, and unlike
  unconfoundedness it is \textbf{checkable}. It is asymmetric: the
  \(ATT\) needs only \(e(X) < 1\), the \(ATE\) both ends.
\item
...
\item
  ...
  \(Y^0_i\) with a comparable control's observed outcome. Exact matching
  is the honest version, and it fails on continuous \(X\).
\item
  \textbf{Nearest-neighbour matching} buys feasibility with a
  \textbf{matching discrepancy} that is \emph{signed}, not noise:
  treated units with high \(X\) are matched downward. It is bias.
\item
  ...
  is invalid.
\item
  \textbf{Two separate repairs.} The \emph{variance} estimator fixes
  inference: matching with replacement makes comparisons dependent, and
  \(K_j(K_j-1)\) prices that in. \emph{Bias correction} fixes the
  estimate, using a regression only to price the gap
  \(\hat\mu(X_i) - \hat\mu(X_{j(i)})\) --- far weaker than modelling the
  level of \(Y\).
\item
  \textbf{Scale matters.} Euclidean and Mahalanobis are different
  estimators, not different implementations: on the training data they
  return \(1607.5\) and \(1315.0\).
\item
