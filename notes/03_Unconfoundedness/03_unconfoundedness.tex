...
\begin{frame}{Why \emph{that} weight for ATE?}
\protect\phantomsection\label{why-that-weight-for-ate}
We prove \(E\!\left[\frac{A \cdot Y}{e(X)}\right] = E[Y(1)]\). The
\(E[Y(0)]\) half is symmetric.
...
Symmetrically \(E\!\left[\frac{(1 - A) Y}{1 - e(X)}\right] = E[Y(0)]\),
giving the \textbf{IPW form of the ATE}:
...
\begin{frame}{Why \emph{that} weight for ATT?}
\protect\phantomsection\label{why-that-weight-for-att}
Consistency gives the first term free:
\(E[Y(1) \mid A{=}1] = E[Y \mid A{=}1]\). The work is in the second.
...
\item
  ...
  then difference. All the §6 machinery applies directly.
\item
