...
\textbf{Mediator}         & $M$ lifeboat    & \textbf{Forbidden} for the total effect. \textbf{Allowed} for the partial effect. \\
\bottomrule
\end{tabular}
\end{center}
...
- If there’s no old people in the treatment group, then clearly you cannot calculate $SDO_{o}$
...
\begin{tabular}{lcccc}
\toprule
 & Difference in & \multicolumn{3}{c}{Stratification weight $k$} \\
\cmidrule(lr){3-5}
Stratum & survival & $ATE$ & $ATT$ & $ATU$ \\
\midrule
...
- $\hat\sigma^2(X_j) = \mathrm{var}(Y \mid X_j, D{=}0)$, the conditional outcome variance among controls at $X_j$. <!-- Estimate it by matching *controls to controls*: $\tfrac{1}{2}(Y_j - Y_\ell)^2$. -->
...
1. **Match.** Nearest neighbour matching; form $\hat\delta_i = Y_i - Y_{j(i)}$ for each treated unit.
...
- **Matching** is explicit imputation: replace the missing $Y^0_i$ with a comparable control's outcome. Exact matching is the honest version; it fails on continuous $X$.
- **Nearest-neighbour matching** buys feasibility with a **matching discrepancy** that is *signed*, not noise. 
<!-- It is bias, and it shrinks *slower* than the standard error --- $O_p(N^{-1/k})$ against $O_p(N^{-1/2})$, so from $k \geq 3$ the $t$-statistic is invalid. --> 
- **That bias is a product**: how bad the matches are, times how steep $\mu^0$ is. Either factor at zero kills it.
