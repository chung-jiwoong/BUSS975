...
Recall Lecture 1's decomposition. It is an **identity**:
...
- Confusing them is the most common error in empirical finance: treating a long control list as an identification strategy.
- Unconfoundedness is this lecture's mechanism: treatment assigned **as if at random within strata of $X$**.
- Everything that follows is a statistical model, credible *only* if you can defend that mechanism.
...
- The data are real, complete, small enough to compute by hand, and the confounders are undisputed.
...
- $S$ predicts the outcome but not treatment: free precision. The honest case for non-confounder controls.
- $M$ is subtle. Conditioning on it asks: *did first class help, holding fixed the main way it helped?* Near zero --- and the wrong question.
...
- Analyst coverage is the finance collider: it responds to *both* the financing event and firm performance.
- The drawn amount is the mediator a referee will *ask* for --- "surely control for how much they borrowed." That removes the channel you are measuring.
- The last row has no Titanic counterpart, and that is the whole problem. On the Titanic we know the confounders; in finance we rarely do.
...
- Rule 2 does most of the work; rule 3 is most often violated in finance.
- "Pre-treatment" does the heavy lifting in rule 2: a timing argument, and timing is checkable.
...
- They coincide only under constant effects. With heterogeneity they differ, sometimes a lot.
...
- Papers routinely estimate the $ATT$ then draw conclusions requiring the $ATE$ or $ATU$. Watch for it in referee reports --- including your own.
...
- The §1 assignment mechanism, stratum by stratum:
$$
\begin{aligned}
...
- Switching estimators cannot rescue a design: all lean on the first equality, none can test it. **Identification** credibility is common to all.
- They differ downstream --- functional form, extrapolation, which strata get weight. Those are **estimation** differences; Lecture 3b §3 returns to them.
...
- Firms may *anticipate their own gains* $Y^1$ and select on them, but not on how they would have done anyway.
  - Often defensible: a firm knows its expected benefit from a credit line better than its counterfactual path without one.
...
- Stratify on $\{$sex, age$\}$: four strata, all occupied.
...
- Composition: $76\%$ of passengers were adult males --- the *smallest* effect --- but only $54\%$ of first class.
- The last stratum holds **one** first-class female child. Remember her.
...
- Non-parametric methods **break down honestly**: they simply cannot compute the estimate.
- Regression **does not break down**: it extrapolates across empty cells using functional form, and reports a number with a standard error.
...
- Most methods impute **implicitly**, inside a formula. Matching does it **explicitly**, unit by unit --- its main virtue.
...
- If two controls sit at the same distance from $i$, use both --- breaking ties arbitrarily injects the analyst's coin flip.
...
- With discrete $X$ and full common support, exact matching and subclassification give the **same answer** --- the same estimator, organized differently.
...
- Term (a) is the **sample variance of the $\hat\tau_i$**. Alone it is the without-replacement formula: with $K_j \in \{0,1\}$, term (b) vanishes.
...
- Scaled by $\sqrt{N_T}$, rows (1) and (2) converge to mean-zero normals: noise, and they average away.
...
- For the $ATT$ it is $Y^0$ that is missing and imputed, so only $\mu^0$ has to be modelled.
...
- So $\hat\mu^0$ must be accurate *over the gap*, not globally. Get the level wrong everywhere and the correction is unaffected: $\hat\alpha$ cancels.
- **This is why bias correction is more credible than regression alone.** Regression extrapolates $\hat\mu^0$ across the whole covariate space; bias correction interpolates across a gap matching already made small.
- Division of labour: **matching does the heavy lifting on comparability; regression cleans up the remainder.**
- The worse your matches, the more work the regression does --- and the more the method degenerates into regression.
...
- Four points and one outlier. The "small" regression is doing enormous work --- the method's exposure.
...
- **Unconfoundedness**, $(Y^1,Y^0) \indep D \mid X$: randomization *within* strata of $X$. Untestable; defended institutionally, and Imbens's third defence is strongest in finance.
...
- **Matching** is explicit imputation: replace the missing $Y^0_i$ with a comparable control's observed outcome. Exact matching is the honest version, and it fails on continuous $X$.
- **Nearest-neighbour matching** buys feasibility with a **matching discrepancy** that is *signed*, not noise: treated units with high $X$ are matched downward. It is bias.
- **The bias shrinks slower than the standard error**: $O_p(N^{-1/k})$ against $O_p(N^{-1/2})$. At $k = 2$ they are the same order; at $k \geq 3$ the bias dominates and the $t$-statistic is invalid.
- **Two separate repairs.** The *variance* estimator fixes inference: matching with replacement makes comparisons dependent, and $K_j(K_j-1)$ prices that in. *Bias correction* fixes the estimate, using a regression only to price the gap $\hat\mu(X_i) - \hat\mu(X_{j(i)})$ --- far weaker than modelling the level of $Y$.
- **Scale matters.** Euclidean and Mahalanobis are different estimators, not different implementations: on the training data they return $1607.5$ and $1315.0$.
