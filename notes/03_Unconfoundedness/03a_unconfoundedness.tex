...
\tightlist
\item
  What deletes the last two terms is a \textbf{claim about how treatment
  was assigned}.
\end{itemize}
\end{frame}
...
\item
  A statistical model, credible \emph{only} if you can defend that
  mechanism.
\item
...
\item
  That claim is \textbf{not testable}.
\item
...
\item
  ...
  independence}, or \textbf{selection on observables}. Same thing.
\end{itemize}
\end{frame}
...
\item
  ...
  holding fixed the main way it helped?}
\item
...
\tightlist
\item
  If there's no old people in the treatment group, then clearly you
  cannot calculate \(SDO_{o}\) and so common support ultimately breaks
  the whole estimation process down.
\end{itemize}
\end{frame}
...
\tightlist
\item
  Each estimate is a \textbf{weighted average of the same four
  differences}, read down its column:
  \(0.757(13.7) + 0.193(34.6) + 0.029(59.3) + 0.020(38.6) = 19.6\) pp.
\item
...
\tightlist
\item
  1 binary covariate: 2 cells. 10 binary: \(2^{10} = 1{,}024\). 1
  continuous: infinitely many.
\item
  Cells grow exponentially; your sample doesn't. Empty cells become the
  norm.
\item
  Non-parametric methods \textbf{fail}: no estimate.
\item
  Regression \textbf{doesn't fail}: it extrapolates across empty cells
  via functional form --- and reports a number with a standard error.
\end{itemize}
\end{frame}
...
\item
  ...
  does it \textbf{explicitly}, unit by unit.
\item
...
\item
  ...
  subclassification give the \textbf{same answer}.
\item
  The difference: subclassification aggregates to stratum effects first;
  matching imputes unit by unit.
\item
...
\item
  ...
  (b) vanishes.
\item
  \(\hat\sigma^2(X_j) = \mathrm{var}(Y \mid X_j, D{=}0)\), the
  conditional outcome variance among controls at \(X_j\).
\item
...
Matching was only \emph{approximate}: \(X_{j(i)} \approx X_i\). What
does the leftover gap cost?
...
\tightlist
\item
  \textbf{Match.} Nearest neighbour matching; form
...
\item
  One outlier. The ``small'' regression is doing enormous work --- the
  method's exposure.
\end{itemize}
\end{frame}
...
\item
  ...
  colliders (never), precision covariates (optional), mediators
  (\emph{depends on the estimand}: forbidden for the total effect,
  required for the partial). Pre-treatment timing is your best
  protection.
\item
...
\item
  ...
  nuisance. One passenger of \(2{,}201\) killed the \(ATE\) and \(ATU\);
  the \(ATT\) survived.
\end{itemize}
\end{frame}
...
\item
  ...
  \(Y^0_i\) with a comparable control's outcome. Exact matching is the
  honest version; it fails on continuous \(X\).
\item
  \textbf{Nearest-neighbour matching} buys feasibility with a
  \textbf{matching discrepancy} that is \emph{signed}, not noise.
\item
  \textbf{That bias is a product}: how bad the matches are, times how
  steep \(\mu^0\) is. Either factor at zero kills it.
\item
  \textbf{Two separate repairs.} The \emph{variance} estimator fixes
  inference: reusing controls makes comparisons dependent, and
  \(K_j(K_j-1)\) prices that in. \emph{Bias correction} fixes the
  estimate, subtracting a modelled
  \(\hat\mu^0(X_i) - \hat\mu^0(X_{j(i)})\).
\item
  \textbf{Scale matters.} Euclidean and Mahalanobis are different
  estimators, not different implementations: \(1607.5\) against
  \(1315.0\), truth \(1607.5\).
\item
  \textbf{Report raw and bias-corrected side by side.} A large gap says
  the matches were poor and the outcome model is load-bearing.
\item
